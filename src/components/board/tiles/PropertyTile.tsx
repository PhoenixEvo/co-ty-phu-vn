...
export default function PropertyTile({ space, edge, owner }: PropertyTileProps) {
  ...
  const getNameFontSize = (name: string) => {
    ...
    if (len <= 7) return 'text-[9px] md:text-[10.5px] font-extrabold';
    if (len <= 13) return 'text-[8px] md:text-[9.5px] font-bold leading-[1.12]';
    return 'text-[7px] md:text-[8px] font-bold leading-[1.08]';
  };
  ...
  if (edge === 'bottom') {
    return (
      <div className="w-full h-full flex flex-col justify-between bg-white relative select-none">
        ...
        <div className={`h-[20%] w-full ${colorClass} shrink-0 flex items-center justify-center relative shadow-2xs`}>
          {owner && (
            <div 
  ...
  if (edge === 'left') {
    return (
      <div className="w-full h-full flex flex-row justify-between bg-white relative select-none">
        ...
        <div className={`w-[20%] h-full ${colorClass} shrink-0 flex items-center justify-center relative shadow-2xs`}>
          {owner && (
            <div 
              className="w-2.5 h-2.5 rounded-full border border-white shadow-xs"
              style={{ backgroundColor: owner.tokenColor }}
